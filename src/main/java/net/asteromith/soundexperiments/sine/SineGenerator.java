 ...
 */
package net.asteromith.soundexperiments.sine;
...
public class SineGenerator {
  ...
  public static void main(String[] args) throws Exception {
    float frequency = parseFrequency(args);
    int durationMs = parseDuration(args);
    ...
    try (var line = AudioSystem.getSourceDataLine(CDDA_MONO)) {
      ...
      for (var i = 0; i < durationMs * CDDA_SAMPLE_RATE / 1000; i++) {
        ...
        var value = (Math.sin(angle) * 100);
        buf[1] = (byte) ((int) value & 0xff00);
        buf[0] = (byte) ((int) value & 0x00ff);
        line.write(buf, 0, 2);
      }
      line.drain();
      line.stop();
